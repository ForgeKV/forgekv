...
fn path_set(
    ...
    if path == "$" || path == "." {
        ...
        if nx && existed { return Some(false); }
        if xx && !existed { return Some(false); }
        *root = new_val;
        return Some(!existed);
    }
    let path = if path.starts_with("$.") { &path[2..] }
               else if path.starts_with('$') { &path[1..] }
               else if path.starts_with('.') { &path[1..] }
               else { path };
...
impl CommandHandler for JsonSetCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("WRONGTYPE") };
        let path = match args[2].as_str() { Some(s) => s.to_string(), None => return RespValue::error("ERR invalid path") };
        let json_str = match args[3].as_str() { Some(s) => s, None => return RespValue::error("ERR value must be a string") };
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        while i < args.len() {
            match args[i].as_str().map(|s| s.to_uppercase()).as_deref() {
                Some("INDENT") | Some("NEWLINE") | Some("SPACE") => { i += 2; continue; }
                _ => {}
            }
        ...
        if paths.len() == 1 {
            ...
            if paths[0] == "$" || paths[0] == "." {
                return RespValue::bulk_str(&serde_json::to_string(&results[0]).unwrap_or_default());
            }
            // JSONPath style returns array
            let json = serde_json::to_string(&serde_json::Value::Array(results)).unwrap_or_default();
            ...
            RespValue::bulk_str(&serde_json::to_string(&serde_json::Value::Object(obj)).unwrap_or_default())
        }
    }
}
...
impl CommandHandler for JsonMGetCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        let db = *db_index;
        let results: Vec<RespValue> = args[1..args.len()-1].iter().map(|a| {
            let key = match a.as_bytes() { Some(b) => b.to_vec(), None => return RespValue::null_bulk() };
            match store_get(db, &key) {
                ...
                Some(root) => {
                    ...
                    if vals.is_empty() {
                        ...
                        let json = serde_json::to_string(&serde_json::Value::Array(vals)).unwrap_or_default();
                        RespValue::bulk_str(&json)
                    }
                }
            }
...
pub struct JsonDelCommand { pub name: &'static str }
impl CommandHandler for JsonDelCommand {
    fn name(&self) -> &str { self.name }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 2 {
            return RespValue::error(&format!("ERR wrong number of arguments for '{}'", self.name));
        }
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("WRONGTYPE") };
...
impl CommandHandler for JsonTypeCommand {
    fn name(&self) -> &str { "JSON.TYPE" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 2 {
            return RespValue::error("ERR wrong number of arguments for 'JSON.TYPE'");
        }
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::null_bulk() };
        let path = args.get(2).and_then(|a| a.as_str()).unwrap_or("$");
        let db = *db_index;
        match store_get(db, &key) {
            None => RespValue::null_bulk(),
            Some(root) => {
                let vals = path_get(&root, path);
                let types: Vec<RespValue> = vals.iter().map(|v| {
                    RespValue::bulk_str(json_type_name(v))
                }).collect();
                if types.is_empty() {
                    RespValue::null_bulk()
                } else if path == "$" || path == "." {
...
fn json_type_name(v: &serde_json::Value) -> &'static str {
    match v {
        ...
        serde_json::Value::Number(n) => if n.is_f64() { "number" } else { "integer" },
...
impl CommandHandler for JsonNumIncrByCommand {
    fn name(&self) -> &str { "JSON.NUMINCRBY" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 4 {
...
impl CommandHandler for JsonNumMultByCommand {
    fn name(&self) -> &str { "JSON.NUMMULTBY" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 4 {
...
fn num_op_path<F: Fn(f64) -> f64>(
    ...
    let canonical = if path.starts_with("$.") { &path[2..] }
                    else if path.starts_with('$') { &path[1..] }
                    else if path.starts_with('.') { &path[1..] }
                    else { path };
...
impl CommandHandler for JsonStrLenCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        match store_get(db, &key) {
            ...
            Some(root) => {
                let vals = path_get(&root, path);
                let results: Vec<RespValue> = vals.iter().map(|v| {
                    match v.as_str() {
...
impl CommandHandler for JsonArrAppendCommand {
    fn name(&self) -> &str { "JSON.ARRAPPEND" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 4 {
        ...
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("WRONGTYPE") };
        let path = match args[2].as_str() { Some(s) => s.to_string(), None => return RespValue::error("ERR invalid path") };
        let new_items: Vec<serde_json::Value> = args[3..].iter().filter_map(|a| {
            a.as_str().and_then(|s| serde_json::from_str(s).ok())
        }).collect();
        let db = *db_index;
        let mut root = match store_get(db, &key) {
            Some(v) => v,
        ...
        let canonical = if path.starts_with("$.") { path[2..].to_string() }
                        else if path.starts_with('$') { path[1..].to_string() }
                        else if path.starts_with('.') { path[1..].to_string() }
                        else { path.clone() };
...
fn json_clear_path(root: &mut serde_json::Value, path: &str) -> usize {
    if path == "$" || path == "." {
        let mut count = 0;
        if let Some(arr) = root.as_array_mut() { arr.clear(); count += 1; }
        else if let Some(obj) = root.as_object_mut() { obj.clear(); count += 1; }
        else if root.is_number() { *root = serde_json::Value::Number(0.into()); count += 1; }
        return count;
    }
    let vals = path_get(root, path);
    vals.len()
...
impl CommandHandler for JsonRespCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        match store_get(db, &key) {
            ...
            Some(root) => {
                let vals = path_get(&root, path);
                if vals.is_empty() { return RespValue::null_bulk(); }
                json_to_resp(&vals[0])
            }
        }
    }
...
fn json_to_resp(val: &serde_json::Value) -> RespValue {
    match val {
        ...
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() { RespValue::Integer(i) }
            else { RespValue::bulk_str(&n.to_string()) }
        }
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        match args[1].as_str().map(|s| s.to_uppercase()).as_deref() {
            Some("MEMORY") => {
                ...
                let key = match args[2].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::Integer(0) };
                let db = *db_index;
                match store_get(db, &key) {
            ...
            Some("HELP") => RespValue::Array(Some(vec![
                RespValue::bulk_str("JSON.DEBUG MEMORY <key> [path] - Report memory usage"),
            ])),
...
impl CommandHandler for JsonMSetCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        let mut i = 1;
        while i + 2 < args.len() {
            let key = match args[i].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("ERR key must be a string") };
            let path = match args[i+1].as_str() { Some(s) => s.to_string(), None => return RespValue::error("ERR path must be a string") };
            let json_str = match args[i+2].as_str() { Some(s) => s, None => return RespValue::error("ERR value must be a string") };
            let new_val: serde_json::Value = match serde_json::from_str(json_str) {
                Ok(v) => v,
                Err(e) => return RespValue::error(&format!("ERR invalid JSON: {}", e)),
            };
...
impl CommandHandler for JsonStrAppendCommand {
    fn name(&self) -> &str { "JSON.STRAPPEND" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 3 {
        ...
        let (path, json_str) = if args.len() == 3 {
            ...
            (args[2].as_str().unwrap_or("$").to_string(), args[3].as_str().unwrap_or("").to_string())
        };
        ...
        store_set(db, key, root);
        if lengths.len() == 1 { lengths.into_iter().next().unwrap() }
        else { RespValue::Array(Some(lengths)) }
    }
}
...
impl CommandHandler for JsonArrPopCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        let path = if args.len() >= 3 { args[2].as_str().unwrap_or("$").to_string() } else { "$".to_string() };
        let index: i64 = if args.len() >= 4 { args[3].as_str().and_then(|s| s.parse().ok()).unwrap_or(-1) } else { -1 };
        let mut root = match store_get(db, &key) {
            Some(v) => v,
            None => return RespValue::null_bulk(),
        };
...
fn json_get_mut<'a>(root: &'a mut serde_json::Value, path: &str) -> Option<&'a mut serde_json::Value> {
    if path == "$" || path == "." { return Some(root); }
    let path = if path.starts_with("$.") { &path[2..] }
               else if path.starts_with('$') { &path[1..] }
               else if path.starts_with('.') { &path[1..] }
               else { path };
...
fn json_get_mut_inner<'a>(val: &'a mut serde_json::Value, path: &str) -> Option<&'a mut serde_json::Value> {
    ...
    let (field, rest) = if let Some(dot) = path.find('.') {
        if let Some(bracket) = path.find('[') {
            if bracket < dot { (&path[..bracket], &path[bracket..]) }
            else { (&path[..dot], &path[dot+1..]) }
        } else { (&path[..dot], &path[dot+1..]) }
    } else if let Some(bracket) = path.find('[') {
        (&path[..bracket], &path[bracket..])
    } else { (path, "") };
...
impl CommandHandler for JsonArrIndexCommand {
    fn name(&self) -> &str { "JSON.ARRINDEX" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 4 {
        ...
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("ERR") };
        let path = args[2].as_str().unwrap_or("$").to_string();
        let needle_str = match args[3].as_str() { Some(s) => s, None => return RespValue::error("ERR") };
        let needle: serde_json::Value = match serde_json::from_str(needle_str) {
            Ok(v) => v,
            Err(_) => return RespValue::error("ERR invalid JSON"),
        };
        let start: usize = args.get(4).and_then(|a| a.as_str()).and_then(|s| s.parse().ok()).unwrap_or(0);
        let stop: i64 = args.get(5).and_then(|a| a.as_str()).and_then(|s| s.parse().ok()).unwrap_or(0);
        let root = match store_get(db, &key) {
            Some(v) => v,
            None => return RespValue::null_bulk(),
        };
        let targets = path_get(&root, &path);
        let results: Vec<RespValue> = targets.iter().map(|v| {
            if let Some(arr) = v.as_array() {
                let end = if stop <= 0 { arr.len() } else { (stop as usize).min(arr.len()) };
                for (i, elem) in arr.iter().enumerate().skip(start) {
                    if i >= end { break; }
                    if elem == &needle { return RespValue::integer(i as i64); }
                }
                RespValue::integer(-1)
            } else {
                RespValue::null_bulk()
...
impl CommandHandler for JsonArrInsertCommand {
    fn name(&self) -> &str { "JSON.ARRINSERT" }
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        if args.len() < 5 {
        ...
        let values: Vec<serde_json::Value> = args[4..].iter().filter_map(|a| {
            a.as_str().and_then(|s| serde_json::from_str(s).ok())
        }).collect();
        if values.is_empty() { return RespValue::error("ERR no values to insert"); }
        let mut root = match store_get(db, &key) {
            Some(v) => v,
            None => return RespValue::null_bulk(),
        };
        let result = if let Some(target) = json_get_mut(&mut root, &path) {
            if let Some(arr) = target.as_array_mut() {
                let len = arr.len() as i64;
                let idx = if index < 0 { (len + index + 1).max(0) as usize } else { (index as usize).min(arr.len()) };
...
impl CommandHandler for JsonMergeCommand {
    ...
    fn execute(&self, db_index: &mut usize, args: &[RespValue]) -> RespValue {
        ...
        let key = match args[1].as_bytes() { Some(b) => b.to_vec(), None => return RespValue::error("ERR") };
        let path = args[2].as_str().unwrap_or("$").to_string();
        let json_str = match args[3].as_str() { Some(s) => s, None => return RespValue::error("ERR") };
