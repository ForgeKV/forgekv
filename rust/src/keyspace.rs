...
impl KsConfig {
    fn update_enabled(&mut self) {
        let any_type = self.generic || self.string || self.list
            || self.set || self.zset || self.hash || self.expired || self.stream;
...
impl KeyspaceNotifier {
    fn new() -> Self {
        ...
        KeyspaceNotifier { tx, cfg: RwLock::new(KsConfig::default()) }
    }
    ...
    pub fn notify(&self, db: usize, key: &[u8], event: &str) {
        if !self.cfg.read().enabled { return; }
